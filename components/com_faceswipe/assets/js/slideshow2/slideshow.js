...
(function(){
	...
	*/
		...
		next: function(last){
			var n = last ? this.data.images.length - 1 : ((this.slide + 1 + this.data.images.length) % this.data.images.length);
			this.go(n, 'left');
		},
	...
	var Thumbnails = new Class({
		...
		initialize: function(slideshow){
			...
			slideshow.data.thumbnails.each(function(thumbnail, i){
				...
				new Asset.image(thumbnail, {
					width: this.options.image.width,
		...
		onload: function(i){
			...
			var units = Math.floor(div[width] / li[width]),
				...
				len = x * li[length],
				ul = thumbnails.getElement('ul').setStyles({length: len});
			ul.getElements('li').setStyles({'height': li.height, 'width': li.width});
			ul.setStyles({'width': li.height * this.data.images.length});
			thumbnails.setStyles({'bottom': -li.height, 'height': li.height});
			thumbnails.store('limit', div[length] - len);
			this.el.getParent().setStyles({'height': this.height + li.height});
		},
